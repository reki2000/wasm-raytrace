...
>
> **Phase 2(準備フェーズの並列化)は調査の上、見送り。** 実装前に
> `skin()`/BVH(refit)/`packTris()` を個別プロファイリング(node、500回平均)
> したところ、skin 0.59ms + BVH 1.19ms + packTris 0.06ms + JS側
> `GLB.sampleBones()`×6体 0.92ms で合計約2.8ms/frame と、フレーム全体
> (98〜226ms)に対して無視できる小ささだった。一方、実ブラウザで計測
> ウィンドウを揃えて再計測すると mesh 経路 960×540 固定で ST 226ms →
> MT(4参加者) 98ms(2.3×)と、headless の3.4〜3.6×(ほぼ線形)より
> 明確に低い。準備フェーズが2.8msしかない以上この差は自前コードの逐次処理
> が原因ではなく、このサンドボックス環境で実OSスレッド(Worker)を動かす際の
> スケジューリング/コンテンション(Chromiumのコンポジタ/GPUプロセス等と
> 有限vCPUを取り合う)に起因する可能性が高く、コード側の並列化では解決
> できない。skin/BVH/packTrisをさらに並列化しても理論上の上限効果は
> 1〜2ms/frame程度であり、BVH refitのleaf/internal-node依存関係を含む
> 実装リスクに見合わないと判断し、**PR1+PR2をこの取り組みの完了点とする**。
