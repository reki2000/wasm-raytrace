...
| エクスポート API | `dino.c:612-616` — `fb()`(フレームバッファ先頭ポインタ)と `render(t, az, el, dist, w, h)` の2つだけ |
...
### 8. レンダリングパイプライン(`dino.c:615-880` `render()`)

処理順に:

| ステージ | 実装箇所 | 内容 |
|---|---|---|
| カメラ | `dino.c:619-629` | 注視点固定のオービットカメラ(az/el/dist)、焦点距離 FL=1.8 |
| レイ生成 | `dino.c:638-644` | 横4ピクセルを1パケットに(`X4 = (0,1,2,3)`) |
| 個体境界球 | `dino.c:650-669` | 3個体の球とレイの交差。全ミスなら即スキップ |
| プライマリマーチ | `dino.c:671-692` | `buildList` で絞ったリストに対し **64 iter** の sphere tracing。ステップ 0.92 倍の緩和、ヒット閾値は距離比例 |
| 法線・AO | `dino.c:701-716` | **四面体法線**(4 サンプル)、AO は法線方向 1 tap |
| albedo | `dino.c:718-731` | 地面 `groundAlbedo()`(`:466-477`、チェッカー + grime ノイズ、SCROLL でスクロール)と恐竜 `dinoAlbedo()` を hit マスクで合成 |
| ライティング | `dino.c:733-754` | 拡散(太陽1灯)+ 半球アンビエント + Blinn-Phong スペキュラ(目 1.4 / 恐竜 0.35 / 床 0.05) |
| ソフトシャドウ | `dino.c:572-609` `softshadow()` | 3個体の境界球の min entry / max exit で**影レイ区間を統合**(相互影が付く)。12 iter、`min(res, 9·d/s)` の古典式 |
| 体表の環境反射 | `dino.c:756-782` | 反射方向の**解析スカイ + 地面チェッカー**をフレネル `F = 0.03 + 0.55·(1−cosθ)⁵` で加算(再マーチなし)。骨色部位(角・嘴・スパイク)は光沢 1.5 倍(`:777`) |
| 床反射(実マーチ) | `dino.c:783-860` | 地面ヒット点から Y 反転レイで群れを**再マーチ(30 iter)**、影なし簡易シェーディング。ミス時は雲入り解析スカイ。フレネル `kR = 0.14 + 0.60·(1−cosθ)⁵` でブレンド、影の中は減光(`:856`) |
| 霧・空 | `dino.c:862-869` | 距離²ベースの霧、ミスピクセルは `skyCol()`(`:438-464`、グラデ + 太陽グレア + 2オクターブ雲) |
| 出力 | `dino.c:871-877` | ガンマ 2.0(= sqrt)、RGBA8 パックして `FB` に SIMD ストア |

### 9. テクスチャ / 種別の見た目(`dino.c:479-570`)

- `dinoMasks()`(`:479-504`): マーチングに使ったプリミティブリストを個体ごとに実 SDF 評価し、最小距離の個体で種マスク m0/m1/m2 を作る(ヒットした部位のプリミティブは距離 ≈ 0 なので必ず正しい個体が選ばれる)。境界球中心への距離比較では、体の末端(ステゴの頭など)が隣の個体に誤帰属して色が化けるため不可。
- `dinoAlbedo()`(`:507-570`): 種別パレット — 獣脚類 = 緑 + 背縞、ステゴ = 暖褐 + **背板の赤橙グラデ**(板ジオメトリを解析再評価する厳密判定 `PLT[]` `:526-539`。高さパラメータ h からグラデーション)、トリケラ = 灰緑 + フリル同心リング + 暖色リム。共通で 2 オクターブ値ノイズ変調 + 腹の明色化(`:561-565`)。
...
- **速度/品質**: プライマリ 64 / 反射 30 / 影 12 の各 iter 数。反射の有無は `if (any(gm))` ブロック(`dino.c:783`)
