// test_mt_scene.js — headless correctness + scaling check for the
// multithreaded combined-scene row renderer (scenePrep + sceneRowsSteal),
// mirroring test_mt.js / test_mt_mesh.js but exercising the SDF+mesh
// combined path so both renderers' row-stealing cooperation is verified,
...
const SPACING = 3.4;
const W = 480, H = 270;
const T = 0.6, AZ = 0.95, EL = 0.30, DIST = 10.0;
...
function packAll(e, mem){
  ...
  // a mix of materials on both paths so fresnel/metal/acrylic code is
  // exercised in the byte-identical comparison, not just the plain mode
  e.mat(0, 3, 0.35, 0.85, 1.49, 0.0, 0.6);        // SDF theropod: acrylic
  e.mat(1, 2, 0.85, 0.65, 1.49, 0.0, 0.9);        // SDF stego: metal
  e.meshMat(0,2,0.85,0.65,1.49,0.0,0.9,1);        // mesh T-Rex: metallic
  e.meshMat(3,3,0.35,0.85,1.49,0.0,0.6,0);        // mesh Stego: acrylic
  e.meshSetFocus(0,0);
  return built;
}
...
async function renderSceneST() {
  ...
  e.renderScene(T, AZ, EL, DIST, W, H);
...
async function makeMtSession(numWorkers) {
  ...
  async function renderFrame(t, az, el, dist, w, h) {
    ...
    for (const wk of workers) wk.postMessage({ type: 'go', fn: 'sceneRowsSteal', args });
    e.sceneRowsSteal(...args);
...
(async () => {
  console.log(`combined scene: correctness + scaling check, ${W}x${H}, nproc=${require('os').cpus().length}`);

  const stFb = await renderSceneST();
  ...
  console.log(`\nscaling (${N} frames each, ${W}x${H}):`);
  {
    ...
    e.renderScene(0, AZ, EL, DIST, W, H);
    const t0 = process.hrtime.bigint();
    for (let i = 0; i < N; i++){ pushBones(e, e.memory.buffer, built, i/30); e.renderScene(i/30, AZ, EL, DIST, W, H); }
